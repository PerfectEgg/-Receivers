    ...
    public abstract class PacketSession : Session
    {
        ...
        public sealed override int OnRecv(ArraySegment<byte> buffer)
        {
            ...
            while (true)
            {
                ...
                ushort dateSize = BitConverter.ToUInt16(buffer.Array, buffer.Offset);
                if (buffer.Count < dateSize)
                    break;

                OnRecvPacket(new ArraySegment<byte>(buffer.Array, buffer.Offset, dateSize));
                packetCount++;

                processLen += dateSize;
                buffer = new ArraySegment<byte>(buffer.Array, buffer.Offset + dateSize, buffer.Count - dateSize);
            }
